...
export interface CreateSubcategoryFormProps {
  parentCategory: { id_categoria: string; nome: string };
  nome?: string;
  onNomeChange?: (value: string) => void;
  onCancel?: () => void;
  onSubmit?: (data: { nome: string; parent_id: string; adicionado_por: string }) => void;
}
...
export function CreateSubcategoryForm({
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      await onSubmit?.({
        ...
        parent_id: parentCategory.id_categoria,
  ...
  }, [formData, validateForm, onSubmit, parentCategory.id_categoria, onNomeChange]);
    ...
    <Surface className="w-full max-w-md">
      ...
      >
        ...
        <div className="text-sm text-gray-600 dark:text-gray-400">
          Criando subcategoria para: <span className="font-semibold">{parentCategory.nome}</span>
        </div>
        ...
        <div className="flex gap-2 mt-6">
          <Button 
            ...
            isDisabled={isSubmitting}
          >
            {isSubmitting ? "A criar..." : "Criar Subcategoria"}
          </Button>
          {onCancel && (
            <Button 
              type="button" 
